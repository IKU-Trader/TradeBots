...
OHLCV_FILE_DIR = 'ohlcv_files/'      # 保存フォルダ。最後にスラッシュ
...
def bf_get_ohlcv(before_time:int=0):
    ...
    try:
        ...
        if before_time != 0:
            res = _query('https://lightchart.bitflyer.com/api/ohlc?symbol=FX_BTC_JPY&before=' + str(before_time)+ '&period=m', headers)
        else:
            res = _query('https://lightchart.bitflyer.com/api/ohlc?symbol=FX_BTC_JPY&period=m', headers)
        # responce = requ(f"https://lightchart.bitflyer.com/api/ohlc?symbol={self._product}&period=m", headers=headers).json()
        # [0] : time
        # [1] : open
        # [2] : high
        # [3] : low
...
# -----------------------------------------------------------------------------
def get_ohlcv(year:int, start_jst_dt:datetime, end_jst_dt:datetime):
    """
    ...
    while True:
        ...
        if ret_end_dt <= start_jst_dt:
            ...
            # ファイル書き込み
            filename = 'bitflyer_m1_{:02}_{:02}.csv'.format(year, now_month)
        ...
        if ret_end_dt.month != now_month:
            ...
            # ファイル書き込み
            filename = 'bitflyer_m1_{:02}_{:02}.csv'.format(now_year, now_month)
            with open(OHLCV_FILE_DIR+filename, 'w', newline="") as f:
                writer = csv.writer(f, delimiter=",")
                writer.writerow(["date","open","high","low","close","volume","buy_vol","sell_vol"])
                writer.writerows(new_ohlcv_list)
...
def main(year):
    ...
    tbegin = datetime(year, 1, 1, 0, 0)
    tbegin = tbegin.astimezone(JST)
    if year == datetime.now().year:
        tend = 0
    else:
        tend = datetime(year, 12, 31, 23, 59)
        tend = tend.astimezone(JST)
    get_ohlcv(year, tbegin, tend)
